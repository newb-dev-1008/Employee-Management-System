...
# Argparse arguments
savingMultiple = "" # Boolean value to check if a new user is being added or multiple users are being added
if (~savingMultiple):
  name = "" # Name of person
else:
...
embeddingsFolder = "" # Folder containing subfolders with each user's name, further containing pickled files
mainEmbeddingsPath = ""
datasetFolder = ""
...
if (~savingMultiple): 
  ...
  if (len(os.listdir(embeddingsFolder)) == 0):
    ...
    userEmbeddingsPath = userDatasetFolder + "/embeddings.pickle"
...
savePathRecognizer = "" # Enter save path for model folder
savePathLabels = "" # Enter save path for labels folder
